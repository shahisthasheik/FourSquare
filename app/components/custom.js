...
const CustomTabBar = createReactClass({
  ...
  updateTabPanel(position, pageOffset) {
    ...
    position =
      Platform.OS === "ios" && I18nManager.isRTL
        ? this.props.tabs.length - 1 - position
        : position; // fix done for RTL scroll focus skd
    const tabWidth =
      (this._tabsMeasurements[position] &&
        this._tabsMeasurements[position].width) ||
      0;
    ...
    const tabOffset =
      (this._tabsMeasurements[position] &&
        this._tabsMeasurements[position].left) ||
      0;
  ...
  updateTabUnderline(position, pageOffset, tabCount) {
    ...
    if (position < tabCount - 1) {
      ...
      this.state._widthTabUnderline.setValue(
        ...
        //newLineRight - newLineLeft - scale(20)
      );
    } else {
      this.state._leftTabUnderline.setValue(lineLeft);
      this.state._widthTabUnderline.setValue(
        // lineRight - lineLeft - scale(20)
        lineRight - lineLeft
      );
    }
  },
  ...
  renderTab(name, page, isTabActive, onPressHandler, onLayoutHandler) {
    ...
    return (
      ...
      >
        <View
          style={[styles.tab, this.props.tabStyle, { borderColor: textColor }]}
        >
          <Text
            style={[
              {
